...
ssl_tcp_stream::ssl_tcp_stream(std::shared_ptr<tcp_socket> tcp_socket, 
    std::shared_ptr<asio::ssl::context> ssl_context_arg)
...
int managed_ssl_tcp_stream::create_object(lua_State * L)
{
    ...
    lua::create_object<managed_ssl_tcp_stream>(L, 
        std::shared_ptr<ssl_tcp_stream>(new ssl_tcp_stream(socket, ctx)));
...
{
    lua_pushinteger(L, ssl::stream<asio::ip::tcp::socket &>::client);
    lua_setfield(L, -2, "HANDSHAKE_CLIENT");
    
    lua_pushinteger(L, ssl::stream<asio::ip::tcp::socket &>::server);
...
int managed_ssl_tcp_stream::async_handshake(lua_State * L)
{
    ...
    self->stream.async_handshake(
        static_cast<ssl::stream<asio::ip::tcp::socket &>::handshake_type>(handshake), 
        std::bind(async_handshake_handler, self, L, lua::weak_ref::create(L), std::placeholders::_1));
...
int managed_ssl_tcp_stream::async_send(lua_State * L)
{
    ...
    self->stream.async_write_some(asio::buffer(string_copy, string_len),
        protect(std::bind(async_send_handler, self, L, string_copy, callback_ref, std::placeholders::_1, std::placeholders::_2)));
...
int managed_ssl_tcp_stream::async_shutdown(lua_State * L)
{
    ...
    self->stream.async_shutdown(std::bind(async_shutdown_handler, 
        self, L, lua::weak_ref::create(L), std::placeholders::_1));
