...
local function external_load(host, port, db, user, pass, install_db)
  ...
  for tab in mysql.rows(mysql.exec(connection, string.format("SELECT users.name as name, users.pubkey as pubkey FROM users, domains WHERE domains.name = '%s' AND domains.id = users.domain_id", mysql.escape_string(domain))))
	do
