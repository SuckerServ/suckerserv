...
local function use_db(db, arg1, arg2, arg3, arg4, arg5, arg6)
    ...
    if not internal then
        ...
        for domain, users in pairs(domains_and_users) do
            internal.domains[domain] = true
            
            if internal.case_insensitive_domains[domain] then
                for name, prop in pairs(users) do
                for k,v in pairs(prop) do print(k,v) end
                    server.adduser(string.lower(name), domain, prop.key, prop.rights)
                end
            end
            
            for name, prop in pairs(users) do
                server.adduser(name, domain, prop.key, prop.rights)
            end
        end
    else
